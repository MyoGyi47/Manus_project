"""
Core processing logic for Geo Processor    aaaa
"""
import re
import pandas as pd
...
# core_processor.py - get_transformer function အသစ် proj_choice
from config import (
    PROJ_MMD2000_BASE, PROJ_MMD2000_GEO, 
    EPSG_WGS84_GEO, EPSG_WGS84_UTM_BASE
)
from utils import (
    get_all_vertices, move_del_to_remark, detect_zone_from_filename,
    normalize_for_comparison, round_coordinate_for_phrase,
    extract_kml_from_kmz, kml_coords_to_list, detect_zone, determine_zone, get_zone_info_from_filename
)
import xml.etree.ElementTree as ET
from shapely.geometry import Point, LineString, Polygon

# ==================== COORDINATE TRANSFORMATION ====================
def get_transformer(lon, projection, datum, zone_mode="auto", manual_zone=None, filename=None):
    """
    Get transformer based on 3-layer selection
    """
    # Determine zone (UTM မှပဲ zone လိုတယ်)
    zone = determine_zone(lon, projection, datum, zone_mode, manual_zone, filename)
    
    # Build transformer based on combination
    if projection == "UTM":
        if datum == "WGS84":
            # WGS84 UTM
            epsg_code = EPSG_WGS84_UTM_BASE + zone
            return Transformer.from_crs("EPSG:4326", f"EPSG:{epsg_code}", always_xy=True)
        else:  # MMD2000 UTM
            # Add zone to base string
            proj_string = PROJ_MMD2000_BASE + f" +zone={zone}"
            return Transformer.from_crs("EPSG:4326", CRS.from_proj4(proj_string), always_xy=True)
    else:  # Geographic
        if datum == "WGS84":
            # WGS84 Geographic
            return Transformer.from_crs("EPSG:4326", EPSG_WGS84_GEO, always_xy=True)
        else:  # MMD2000 Geographic
            return Transformer.from_crs("EPSG:4326", CRS.from_proj4(PROJ_MMD2000_GEO), always_xy=True)
    ...
    def process(self, row, source_type):
        ...
        if has_underscore and not any(p.lower() in name.lower() for p in self.protected_names):
            ...
            if len(parts) == 2:
                ...
                # Check if it contains "delete bua" or "del bua"
                object_lower = object_part.lower()
                if 'delete bua' in object_lower or 'deleted bua' in object_lower or 'deleted bua area' in object_lower or 'bua deleted area' in object_lower or 'bua deleted' in object_lower or 'bua delete' in object_lower:
                    # For delete bua/del bua: keep in object column without cutting
                    return parts[0].strip(), object_part, remark_to_return
                else:
                    # For other cases: remove del content
                    if 'del' in object_lower:
...
# parse_kml function ကို update
# parse_kml function ကို update
def parse_kml(kml_content, projection, datum, zone_mode="auto", manual_zone=None, filename=None):
    ...
    for pm in placemarks:
        ...
        # needs_rounding ကို projection နဲ့ datum ပေါ်မူတည်ပြီးဆုံးဖြတ်
        needs_rounding = projection == "UTM"  # UTM ဖြစ်ရင် rounding လုပ်မယ်
        
        lon_main = all_vertices[0][0]
        transformer = get_transformer(lon_main, projection, datum, zone_mode, manual_zone, filename)
...
# ==================== VECTOR FILE PROCESSING ====================
def process_vector(input_path, projection, datum, zone_mode="auto", manual_zone=None):
    ...
    if input_path.lower().endswith(('.kmz', '.kml')):
        if input_path.lower().endswith('.kmz'):
            ...
            with open(input_path, 'r', encoding='utf-8') as f:
                kml_content = f.read()
        return parse_kml(kml_content, projection, datum, zone_mode, manual_zone, input_path)  # datum ထည့်ပို့
    else:
        ...
        for _, row in gdf.iterrows():
            ...
            lon_main = all_vertices[0][0]
            transformer = get_transformer(lon_main, projection, datum, zone_mode, manual_zone, input_path)
...
class ExcelExporter:
    ...
    def __init__(self, proj_choice):
        self.proj_choice = proj_choice  # Legacy parameter: "WGS84_UTM", "Custom_UTM", "WGS84_LatLon", "Custom_LatLon"
