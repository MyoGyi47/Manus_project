# del, delete  တွေကို remark ထဲမှာ ဖျောက်ထားတယ်
import pandas as pd
import numpy as np
...
ROAD_KEYWORDS = ["main road", "main rd", "mainroad", "main-road",
    ...
    'river', 'fish farm', "express way", "expressway", "express-way", "zaung dan", "rarilway",]
...
class GeoProcessorApp(QMainWindow):
    ...
    def process_data(self):
        ...
        # Get zone parameters from UI
        if self.zone_auto_radio.isChecked():
            zone_mode = "auto"
            manual_zone = None
            
            # Check if filename has pattern for auto detection
            input_file = self.input_file_edit.text()
            base_name = os.path.basename(input_file)
            
            # Show info about filename-based detection
            pattern1 = r'\d{4}[\s\-]\d{1,2}'  # 2195 15 or 2195-15
            pattern2 = r'\d{4}\d{2}'          # 219515
            
            if re.search(pattern1, base_name) or re.search(pattern2, base_name):
                self.log_text.append(f"Filename pattern detected: {base_name}")
                self.log_text.append("Using filename-based zone detection...")
